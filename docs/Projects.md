...
- <u> Automated Hedging Oracle Database & Reporting</u> </br>
  Implemented event-driven hedging data pipelines supporting nightly hedge data ingestion and reporting. The process monitored the arrival of trigger files in a network directory utilizing AutoSys file watcher jobs. Upon detection of a trigger file, AutoSys initiated a controlled sequence of dependent jobs that executed Oracle-based ETL procedures.</br>
  The pipelines are designed to be decoupled and fault-tolerant, leveraging AutoSys job dependencies and success conditions to ensure downstream processing only executed after upstream stages completed successfully. This approach reduced unnecessary reruns, improved reliability, and ensured accurate delivery of hedging and actuarial reporting outputs.
